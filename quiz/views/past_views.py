...
def past_question_list(request, subject_id, lesson_id):
    """기출문제 문제풀이 페이지를 렌더링합니다."""
    user = request.user
    user_profile = UserProfile.objects.filter(user=user).first()
    subject = get_object_or_404(Subject, pk=subject_id)
    lesson = get_object_or_404(Lesson, pk=lesson_id, subject=subject)

    session_key = f"used_questions_past_{subject_id}_{lesson_id}"
    all_questions_in_lesson = list(
        Question.objects.filter(subject=subject, lesson=lesson, question_type="past").order_by("year", "number")
    )
    ...
    if not available_questions and all_questions_in_lesson:
        logger.debug("All past questions in lesson %s for subject %s done. Resetting session.", lesson_id, subject_id)
    ...
    if available_questions:
        ...
        selected_questions = available_questions[:num_to_select]
    ...
    logger.debug(
        "Selected past questions IDs: %s, Updated session for %s: %s",
