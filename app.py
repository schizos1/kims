# app.py - 테스트 시 모든 문제 표시 및 객관식 채점 확인
...
app.config['SECRET_KEY'] = 'a_very_secret_key_for_a_super_cool_project_v10_mcq_fix2' 
...
# (PRECONFIGURED_TROPHIES 목록은 이전과 동일)
...
def start_concept_test(concept_id):
    ...
    if request.method == 'POST':
        num_correct = 0
        all_questions_in_test = list(concept.questions) 
        for question in all_questions_in_test:
            submitted_answer = request.form.get(f'answer_{question.id}')
            correct_answer_text = question.answer.strip() if question.answer else ""
            is_correct = (submitted_answer is not None and submitted_answer.strip().lower() == correct_answer_text.lower())
        ...
        total_questions_in_test_count = len(all_questions_in_test)
    ...
    questions_to_display = random.sample(list(concept.questions), k=min(len(list(concept.questions)), 5)) if concept.questions else []
    ...
    for question in all_questions_in_concept:
        ...
        if latest_history:
            results_data.append({
                ...
                'submitted_answer': latest_history.submitted_answer, 
                'correct_answer_num': question.answer, 
                'is_correct': latest_history.is_correct
            })
...
def generate_similar_question_page(original_question_id):
    ...
    if request.method == 'GET':
        try:
            ...
            원본 문제: "{original_question.content}"
            ...
            if "[NEW_QUESTION]" in content:
                try:
                    ...
                    if q_match and o1_match and o2_match and o3_match and o4_match and a_match:
                        ...
                        session['similar_Youtube'] = new_a_text # 오타 발견! similar_answer 로 변경 필요
    ...
    if request.method == 'POST':
        ...
        correct_answer = session.pop('similar_Youtube', None) # 여기도 오타! similar_answer 로 변경 필요
